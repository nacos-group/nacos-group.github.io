...
其次，2.2.0添加了数个新的插件支持，例如 [数据源插件](https://nacos.io/zh-cn/docs/v2/plugin/datasource-plugin)、[自定义环境变量插件](https://nacos.io/zh-cn/docs/v2/plugin/custom-environment-plugin)和`限流管控插件`。

其中[自定义环境变量插件](https://nacos.io/zh-cn/docs/v2/plugin/custom-environment-plugin)是社区需求配置加密数据库密码的需求的升级抽象插件，允许通过插件的方式对nacos服务端启动时的配置进行特殊处理，比如可以配置数据库密码为密文，在插件中进行解密，以此来支持配置加密数据库密码以及后续可能有的其他关于配置特殊处理的需求。
