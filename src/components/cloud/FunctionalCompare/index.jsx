...
import { getDataSource, getPrivateDataSource } from "./BaseData.js";

const tabType = signal("public"); // 默认公共云
...
const FunctionalCompare = ({ url }) => {
  ...
  const handleTabChange = (event) => {
    tabType.value = event.detail.tabType; // 更新 tabType
  };
  ...
  // 根据 tabType 获取不同的数据源
  const getDataSourceByTab = () => {
    if (tabType.value === "private") {
      // 返回私有云相关的数据源
      return getPrivateDataSource(t);
    }
    return getDataSource(t); // 公共云数据源
  };

  const currentDataSource = getDataSourceByTab();
  ...
  return (
    <>
      ...
      >
        <div key={tabType} id='collapse-fold' class="collapse bg-base-200 collapse-arrow bg-gray-02">
          {
