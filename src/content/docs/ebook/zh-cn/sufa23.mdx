...
### 2.服务
   ...
   - SDK 和 Server 之间
      ...
      - 客户端基于当前可用的长链接进行配置的查询，注册，注销，订阅，取消订阅等服务发现领域的RPC 语义接口通信。
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 客户端通信 | sync | 支持 | 支持 | 支持 | 支持 | 无rpc语义 | 无rpc语义 |
|  | future | 支持 | 不支持 | 支持 | 支持 | 无rpc语义 | 无rpc语义 |
|  | callback | 结合 guava实现 | 不支持 | 支持 | 支持<br />(依赖jdk 1.8+ completableFuture） | 无rpc语义 | 无rpc语义 |
| 服务端推送 | sync | 无ack | 支持 | 支持 | 支持 | 无rpc语义 | 无rpc语义 |
|  | future | 不支持 | 支持 | 支持 | 支持 | 无rpc语义 | 无rpc语义 |
|  | callback | 不支持 | 支持 | 支持 | 支持 | 无rpc语义 | 无rpc语义 |
