...
可见性用于决定资源是否应该出现在详情、列表和搜索结果中；鉴权用于决定当前身份是否能读取或写入目标资源。两者会配合工作，但职责不同。更多说明请参考[可见性插件](../../../plugin/visibility-plugin.md)。
...
- 为生产资源启用发布 Pipeline，用于安全扫描、格式校验或合规检查。启用和扩展方式见 [AI 发布 Pipeline 插件](../../../plugin/ai-pipeline-plugin.md)。
- 为强制发布建立审批或审计要求。
