...
可见性影响列表查询、详情查看和下载操作。写操作（编辑、发布、删除等）需要 Owner 身份或显式的写权限。
...
- 在详情页可随时切换 Skill 的 `scope`（PUBLIC ↔ PRIVATE）
- 切换为 PRIVATE 后，非 Owner 用户将无法在列表中看到该 Skill
- 只有 Owner 或具有显式写权限的用户可以修改可见性

### 3.3. 版本标签管理

- 发布时可选择自动更新 `latest` 标签指向新版本
- 在版本时间线中可手动绑定/解绑自定义标签（如 `stable`、`canary`）
- `latest` 标签在客户端查询时作为默认版本解析依据
