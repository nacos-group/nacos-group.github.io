...
AgentSpec 使用统一的 [AI 资源生命周期](./ai-resource-lifecycle.md)。

```text
上传或创建草稿 -> 更新草稿 -> 提交 -> 审核通过后发布为 online -> 下线或重新上线
```

常见管理动作包括：

- 上传 ZIP 包或通过接口创建、更新草稿。已发布的规范包不能直接覆盖，修改时应创建新版本草稿。
- 提交草稿。有已启用且支持 AgentSpec 的 Pipeline 节点时进入 `reviewing`，否则直接发布上线。
- 查看审核结果。通过和拒绝都会进入 `reviewed`；通过后发布，拒绝后先重新编辑再修改，或重新提交以重跑检查。
- 发布或重新上线版本，由服务端自动更新 `latest`；用 `stable` 等自定义标签控制应用侧切换。
- 下线单个版本，或通过启用/禁用开关控制整个 AgentSpec 的可用性。启用资源不会自动上线草稿或已下线版本。
- 设置可见范围、查询版本详情或元数据。

管理员应急强制发布、标签和状态规则见 [AI 资源生命周期](./ai-resource-lifecycle.md)。
...
- 需要控制生产切换时使用 `stable` 等自定义标签；`latest` 由服务端自动维护。
