...
description: This article explains how to enable the authentication (Access Control) feature in Nacos and provides examples.
sidebar:
    order: 7
---
...
<Tabs>
  <TabItem label="非Docker镜像部署">
    ...
    - 配置`server.identity`
      ...
      > 注意：身份识别信息保持节点间一致，长时间不一致可能导致节点间数据不一致。
