...
This document corresponds to version 3.2.0 (development version). The compatibility of upgrading from all Nacos versions to this version is as follows:
...
| 2.0.X ~ 3.1.X | Yes | Upgrading from 2.0.X ~ 3.1.X to 3.2.X is supported, but the database schema has changed. Please compare the [mysql-schema.sql](https://github.com/alibaba/nacos/blob/master/distribution/conf/mysql-schema.sql) file before upgrading and apply the new schema. |
...
If there are schema changes, apply the database alterations first. For example:
