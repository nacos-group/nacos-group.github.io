---
title: Nacos Cluster Nginx Load Balancing Guide
keywords: [Nginx, Load Balancing, Cluster, gRPC]
description: How to use Nginx as a load balancer for a Nacos cluster
sidebar:
    order: 9
---
...
covering the HTTP API (port 8848), Console (port 8080), and gRPC communication (port 9848).

> **Nacos 3.x Port Change**: Starting from Nacos 3.x, the console port defaults to **8080**
> (no longer 8848), and the console path no longer has the `/nacos` prefix.
> The HTTP API port remains 8848.
...
http {
    ...
    # Upstream Nacos API servers (port 8848)
    ...
    # Upstream Nacos console servers (port 8080)
    ...
    server {
        ...
        # Console (Nacos 3.x default port 8080, path without /nacos prefix)
        location / {
...
> **Note**: Nacos uses a fixed offset of **1000** from the HTTP API port for gRPC.
> If your Nacos API port is `8848`, gRPC is on `9848`.
...
3. Access the Nacos console at `http://192.168.190.128:7847` (Nacos 3.x console path is `/`, port 8080)
...
- Nacos 3.x console defaults to port **8080** — ensure `nacos_console_cluster` points to the correct port
- For older Nacos versions (< 3.x), the console port is 8848 with path `/nacos`
