...
| 应用版本 | 选择 Nacos 的版本，目前最新版本为 1.4.2     |
...
- 默认集成了 Mysql 作为数据源。在 **Nacos-server-1.4.2** 组件的环境配置中配置如下环境变量，可以切换到其他外部数据源。
...
- 默认生成了 **Nacos-server-1.4.2** 的数据持久化目录。

![nacos-5](https://static.goodrain.com/wechat/nacos-cluster/nacos-cluster-5.png)

- 默认配置了 **Nacos-server-1.4.2** 的健康检查机制，保障实例故障时自动下线，恢复后自动上线。
